  ...
  if(!isset($error)) {
    ...
    // 上の$error['email']が入っていない時
        if (!isset($error)) {
          ...
          // strtolower() = 文字列全てを小文字にする
          // strtoupper() = 文字列全てを大文字にする
          $ext = strtolower($ext);
          ...
          if ($ext == 'jpg' || $ext == 'png' || $ext == 'gif') {
            # code...
           // 画像のアップロード処理
           // date関数で"確認画面ボタン"を押した時の日付を取得し、ファイル名に文字列連結している
           ...
           //　注意！！！　パーミッションを確認し、staffとeveryoneを「read&write」に書き換える。
           // $_FILES['picture_path']['tmp_name']
           // 一時的に保存される場所 = XAMMPの中の場合はxammpfiles/tempの中
            move_uploaded_file(['picture_path']['tmp_name'],'../picture_path/'.$picture_path);

            // $_SESSION = SESSION変数に入力された値を保存
            // 注意！！！　　　　　　　　　　　　　　　　　　　　　　　　　　$_SESSIONを使用する際、ファイルの1番上にsession_start()関数を書く必要がある。
            $_SESSION['join'] = $_POST;
  ...
  <nav class="navbar navbar-default navbar-fixed-top">
      <div class="container">
          ...
          <div class="navbar-header page-scroll">
              ...
              <a class="navbar-brand" href="new_index.html"><span class="strong-title"><i class="fa fa-twitter-square"></i> Seed SNS</span></a>
          </div>
